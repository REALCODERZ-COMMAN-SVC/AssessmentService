...
//    @Query(nativeQuery = true, value = "SELECT distinct (case when sa.student_assessment_id is null then 'false' else 'true' end) as status,lm.language_name as language, ac.created_date, ac.assessment_id as assessment_id,ac.active as active, ac.assessment_desc as assessment_desc,coalesce((select ac.creation_type), null ,'N/A') as creation_type, ac.language_id as language_id From assessment_creation ac INNER JOIN language_master lm ON lm.language_id=ac.language_id left join student_assessment sa on sa.student_assessment_id=(select sa.student_assessment_id from student_assessment sa where sa.assessment_id=ac.assessment_id order by sa.student_assessment_id desc limit 1) where ac.organization_id=:organizationId order by ac.created_date desc")
