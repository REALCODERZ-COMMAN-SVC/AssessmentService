...
//    public List<LinkedCaseInsensitiveMap> allAssessmentsList(@Param("organizationId") Long organizationId);
    @Query(nativeQuery = true, value = "select ac.assessment_id as assessment_id, (CASE When sa.student_assessment_id is null or false then 'N' ELSE 'Y' END) as directAss,ac.assessment_time_bound as assessmentTimeBound, ac.assessment_desc as assessment_desc,lm.language_name as skills_name,coalesce((select ac.creation_type), null ,'N/A') as creation_type,ac.active as active From assessment_creation ac inner join language_master lm on lm.language_id =ac.language_id left join student_assessment sa on sa.student_assessment_id=(select sa.student_assessment_id from student_assessment sa Where sa.assessment_id=ac.assessment_id and sa.direct_ass =true order by sa.student_assessment_id desc limit 0,1) where ac.organization_id=:organizationId order by ac.assessment_id desc")
    public List<LinkedCaseInsensitiveMap> allAssessmentsList(@Param("organizationId") Long organizationId);
...
//    @Query(nativeQuery = true, value = "SELECT count(*) FROM question_master where topic_id=:topic_id and organization_id=:organizationId")
    ...
    @Query(nativeQuery = true, value = "SELECT ac.assessment_id,ac.assessment_desc,ac.time,ac.language_id FROM assessment_creation ac   Where ac.active ='Y' and ac.assessment_id=:assessmentId")
    public List<LinkedCaseInsensitiveMap> getAssessmentById(@Param("assessmentId") Long assessmentId);
